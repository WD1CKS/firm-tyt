...
/**
  ...
  * @retval status
  */
uint8_t  usbd_cdc_EP0_RxReady (void  *pdev __attribute__((unused)))
{ 
  if (cdcCmd != NO_CMD)
  {
    /* Process the data */
...
/**
  ...
  * @retval status
  */
uint8_t  usbd_cdc_DataIn (void *pdev, uint8_t epnum __attribute__((unused)))
{
  uint16_t USB_Tx_ptr;
...
/**
  ...
  */
#ifdef USE_USB_OTG_HS 
static uint8_t  *USBD_cdc_GetOtherCfgDesc (uint8_t speed __attribute__((unused)), uint16_t *length)
{
